...
CurveEditor::CurveEditor() : atkui::Framework(atkui::Orthographic),
                             mSpline(),
                             mRadius(10.0),
                             mSelected(-1),
                             mButtonState(0),
...
void CurveEditor::scene()
{
  ...
  {
    for (int j = 0; j < mSpline.getNumControlPoints(); j++)
    {
      if (mSpline.getInterpolationType() == "Catmull-Rom")
      {
        drawSphere(mSpline.getControlPoint(j), 10);
        if (j + 1 < mSpline.getNumControlPoints())
        {
      ...
      else if (mSpline.getInterpolationType() == "Hermite")
      {
        if (j % 2 == 1 && j - 1 >= 0)
        {
          drawSphere(mSpline.getControlPoint(j - 1) + mSpline.getControlPoint(j), 10);

          drawLine(mSpline.getControlPoint(j - 1), mSpline.getControlPoint(j - 1) + mSpline.getControlPoint(j));
        }
        else
          drawSphere(mSpline.getControlPoint(j), 10);
      }
  ...
  int num_pts = 200;
  int i = 0;
  for (float t = 0; t <= mSpline.getNumSegments() && i < num_pts; t += (1 / (float)num_pts))
  {
    drawSphere(mSpline.getValue(t), 2);
  }
  if (mMode != REMOVE && !mShowControlPoints)
  {
    for (int j = 0; j < mSpline.getNumControlPoints(); j++)
    {
      if (mSpline.getInterpolationType() == "Catmull-Rom" && j % 3 == 0)
      {
        drawSphere(mSpline.getControlPoint(j), 10);
      }
      else if (mSpline.getInterpolationType() == "Hermite" && j % 2 == 0)
      {
        drawSphere(mSpline.getControlPoint(j), 10);
      }
    }
  }
  if (mMode == REMOVE || mSpline.getInterpolationType() == "Linear")
  {
...
void CurveEditor::drawState()
{
  ...
  {
  case ADD:
    drawText("Mode: ADD", 5, th);
    break;
  case EDIT:
    drawText("Mode: EDIT", 5, th);
    break;
  case REMOVE:
    drawText("Mode: DELETE", 5, th);
    break;
  }
